...
public class ChefPompierEvolueeBis {
	...
	// on l'utilise pour verfier si un robot ne peux rien faire et pose probleme pour la date de la simulation
	Robot RobotTrouverMaisPasChemin = null;
	int AraterPlusieurFois = 0;
	private ArrayList<Robot> NePasPrendreEnConsideration = new ArrayList<Robot>();
	
	public ChefPompierEvolueeBis(DonneesSimulation donnees, Simulateur sim) {
	 ...
	 */
	public void strategieEvolueeBis() {
		...
		while(incendie != null) {
			...
			if(robot != null) {
				...
				// TODO add exception lorsqu'on verse de l'eau avec un reservoir vide
				...
				if(RobotTrouverMaisPasChemin != null ) {
					AraterPlusieurFois++;
					System.out.println(RobotTrouverMaisPasChemin);
					RobotTrouverMaisPasChemin.setDateFinTache(trouverMin(donnees.getRobot(),RobotTrouverMaisPasChemin));
					if(AraterPlusieurFois > 6) {
						AraterPlusieurFois = 0;
						
						 NePasPrendreEnConsideration.add(RobotTrouverMaisPasChemin);
					}
	...
	private long trouverMin(Robot[] robots, Robot robotAEliminer) {
		long min = Long.MAX_VALUE;
		
		for(Robot robot : robots) {
			if(min > robot.getDateFinTache()) {
				if(! robot.equals(robotAEliminer)) {
	...
	private long AffecterUneDate(Robot[] robots) {
		long min = Long.MAX_VALUE;
		for(Robot robot : robots) {
			if(! NePasPrendreEnConsideration.contains(robot)) {
				if(min > robot.getDateFinTache()) {
					min = robot.getDateFinTache();
				}
			}
	  ...
	  private Robot trouverRobot(Incendie incendie, Robot[] robots, long date) {
		...
		for(Robot robot : robots) {
			...
			if(robot.getDateFinTache() < date) {
				...
				RobotTrouverMaisPasChemin =robot;
				if(cheminExiste) {
					RobotTrouverMaisPasChemin =null;
